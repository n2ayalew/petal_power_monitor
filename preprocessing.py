...
class Appliance():
    def __init__(self, name, power_data):
        self.name =  'channel_' + str(name)
        self.power_data = power_data
        self.good_chunks = power_data[(power_data[self.name]>1)]
...
def cluster(x_train,x_test, max_number_clusters):
    ...
    for i in xrange(2,max_number_clusters):
        print("Fitting a KMeans model with %d clusters" % (i))
        ...
        print("Silhouette score with %d clusters: %f" % (i,s_score))
    print("Highest silhouete score of %f achieved with %d clusters\n" % (highest_score,len(centers)))
    return centers
