...
def compute_means_fhmm(means_list):
    """
...
class FHMM():
    ...
    def train(self, appliances, num_states_dict={}, **load_kwargs):
        ...
        for i, app in enumerate(appliances):
            ...
            if num_states_dict.get(app.name) is not None:
                ...
                print("Identifying number of hidden states for appliance " + str(app.name))
                states = cluster(X, max_num_clusters)
                num_total_states = len(states)
                print("Number of hidden states for appliance " + str(app.name) + ": " + str(num_total_states))
            print("Training model for appliance %s with %d hidden states" % (app.name, num_total_states))
            learnt_model[app.name] = hmm.GaussianHMM(num_total_states, "full")
        ...
        for app in learnt_model:
            ...
            new_learnt_models[app] = hmm.GaussianHMM(
                startprob.size, "full")
    ...
    def disaggregate(self, mains, output_datastore, sample_period = 20):
        ...
        **load_kwargs : key word arguments
        ...
        '''
        #down_sampled = mains.groupby(np.arange(len(mains))) // sample_period
